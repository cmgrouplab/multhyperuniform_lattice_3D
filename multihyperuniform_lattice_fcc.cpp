...
#define n_s 5 //number of species for multi-hyperuniformity 
...
void init_config()
{
	...
	//now put in the FCC sites, making -1 meaning to be occupied by other atoms
    for(int i=0; i<MAXX/2; i++)
		for(int j=0; j<MAXX/2; j++)
			for(int k=0; k<MAXX/2; k++)
			{
				config[2*i][2*j][2*k] = -1;
				config[2*i+1][2*j+1][2*k] = -1;

...
//adapted 12/25/24 for FCC HEA in 3D，still using the simple cubic configuration, but a subset of points for FCC
//updated 01/08/25: csv file writer only work up to 5 speices, with generic lables "A, B, C ..."

//updated: 02/06/25: for BCC alloys in 3D, basically just change the basis in the cubic cell
...
#define MAXX 8 //the linear dimension of the lattice, applying to all lattice, w.r.t. the unit cell, HAS TO BE EVEN
               //this leads to 1/2*MAXX*MAXX*MAXX = 500 fcc sites, for 5 species even
#define N_tot MAXX*MAXX*MAXX/4 //the total number of metal particles, the remaining half are carbons

#define n_s 4 //number of species for multi-hyperuniformity 
...
double T = 0.00015; //the current temperature
...
void init_config()
{
	...
	//set all entry to be -10 indicating non-occuptation 
	for(int i=0; i<MAXX; i++)
		for(int j=0; j<MAXX; j++)
			for(int k=0; k<MAXX; k++)
			config[i][j][k] = -10;

	//now put in the BCC sites, making -1 meaning to be occupied by other atoms
	//the translation vector is 2*i, 2*j, 2*k, so just need to put the two basis particles, at 000 and 111
    for(int i=0; i<MAXX/2; i++)
		for(int j=0; j<MAXX/2; j++)
			for(int k=0; k<MAXX/2; k++)
			{
				config[2*i][2*j][2*k] = -1;
				config[2*i+1][2*j+1][2*k+1] = -1;
				//config[2*i+1][2*j][2*k+1] = -1;
				//config[2*i][2*j+1][2*k+1] = -1;
			}
  ...
  for(int s=0; s<n_s; s++)
      ...
      for(int i=sum; i<N_tot; i++)
	{
	  if(Type_c[i] == s)
	    {
		  if(s==0)	
	         fout<<"A,"<<coords[i][0]<<","<<coords[i][1]<<","<<coords[i][2]<<endl;
		  else if(s==1)	
	         fout<<"B,"<<coords[i][0]<<","<<coords[i][1]<<","<<coords[i][2]<<endl;
		  else if(s==2)	
	         fout<<"C,"<<coords[i][0]<<","<<coords[i][1]<<","<<coords[i][2]<<endl;
		  else if(s==3)	
	         fout<<"D,"<<coords[i][0]<<","<<coords[i][1]<<","<<coords[i][2]<<endl;
		  else if(s==4)	
	         fout<<"E,"<<coords[i][0]<<","<<coords[i][1]<<","<<coords[i][2]<<endl;
